...
from .l2norm import tl_l2norm_fwd
...
def main(B=1, S=1024, H=16, D=128):
    ...
    q = tl_l2norm_fwd(q)[0].requires_grad_(True)
    k = tl_l2norm_fwd(k)[0].requires_grad_(True)
    ...
    print(f"dq max err: {(dq - q.grad).abs().max()}")
    print(f"dk max err: {(dk - k.grad).abs().max()}")
    print(f"dv max err: {(dv - v.grad).abs().max()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--D", type=int, default=64, help="Head dim")
